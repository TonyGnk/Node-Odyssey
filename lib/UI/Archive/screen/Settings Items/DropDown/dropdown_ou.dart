...
const double _kMenuHorizontalPadding = 26.0;
...
/// shows a popup menu, consider using [OneUIPopupMenuButton].
///
/// Typically the [child] of a [OneUIPopupMenuItem] is a [Text] widget. More
/// elaborate menus with icons can use a [ListTile]. By default, a
/// [OneUIPopupMenuItem] is 40 pixels high. If you use a widget
/// with a different height, it must be specified in the [height] property.
class OneUIPopupMenuItem<T> extends PopupMenuEntry<T> {
  ...
  @override
  OneUIPopupMenuItemState<T, OneUIPopupMenuItem<T>> createState() =>
      OneUIPopupMenuItemState<T, OneUIPopupMenuItem<T>>();
}
...
class OneUIPopupMenuItemState<T, W extends OneUIPopupMenuItem<T>>
  ...
  ///
  /// By default, this returns [OneUIPopupMenuItem.child]. Override this to put
  ...
  @protected
  void handleTap() {
    //print the current value of the widget
    //if selection is option1 then set selected option1 etc
    //print();
    ...
    return MergeSemantics(
      child: Semantics(
        ...
        child: InkWell(
          ...
          splashFactory: widget.useOneUISplashFactory
    ...
    return FadeTransition(
      ...
      child: ScaleTransition(
        ...
        child: Material(
          ...
          borderRadius: route.borderRadius ?? _kRadius,
          elevation: route.elevation ?? popupMenuTheme.elevation ?? 2.0,
          child: ClipRRect(
            borderRadius: route.borderRadius ?? _kRadius,
            child: Material(
...
class _PopupMenuRoute<T> extends PopupRoute<T> {
  ...
  @override
  Duration get reverseTransitionDuration => _kMenuReverseDuration;
  ...
  Widget buildPage(BuildContext context, Animation<double> animation,
    ...
    return SafeArea(
      child: Builder(
        builder: (BuildContext context) => CustomSingleChildLayout(
...
class _OneUIPopupMenuButtonState<T> extends State<OneUIPopupMenuButton<T>> {
  void showButtonMenu() {
    final PopupMenuThemeData popupMenuTheme = PopupMenuTheme.of(context);
    ...
    if (items.isNotEmpty) {
      unawaited(showMenu<T?>(
        ...
        position: position,
        borderRadius: widget.borderRadius ?? _kRadius,
